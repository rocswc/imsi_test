...
<body>
		...
		<div class="left-section">
			<div class="walking-course">
				...
				<div class="info-form">
					...
					<br>
					<div class="info-item">
						<label for="course-name">산책로</label> 
	...
	function selectCourseFromList(id) {
		...
		for (i = 0; i < allCourses.length; i++) {
			if (allCourses[i].walking_id === parseInt(id)) {
				break;
			}
		}
		...
		document.getElementById("walking-id").value = course.walking_id;
		document.getElementById("course-name").value = course.walking_name;
		document.getElementById("course-distance").value = course.walking_distance;
		document.getElementById("course-info").innerText = course.walking_info;

		var newCenter = new kakao.maps.LatLng(course.walking_latitude, course.walking_longitude);
		...
		$.ajax({
			type: "POST",
			url: "insertWalking",
			data: JSON.stringify({ walking_id: course.walking_id }),
	...
	function translateCardText($card, i, res) {
		$card.find(".course-name").text(res.walking_name || "");
		$card.find(".course-description").text(res.walking_info || "");
		 const rawDistance = (res.walking_distance || "").replace(/[^0-9.]/g, "");
		...
		$(".recommendation-header").find("h2").text("Recommended Walking Courses Nearby");
	...
	navigator.geolocation.getCurrentPosition(function(position) {
		...
		$.ajax({
			type: "GET",
			url: "walkingMap",
			data: { walking_latitude: userLat, walking_longitude: userLon },
			dataType: "json",
		...
		$.ajax({
			...
			success: function(response) {
				...
				for (var i = 0; i < response.length; i++) {
					...
					var $name = $("<h3>").addClass("course-name").text(course.walking_name);
					...
					var distanceText = course.walking_distance ? course.walking_distance + "km" : "정보 없음";
					...
					var $description = $("<div>").addClass("course-description").text(course.walking_info?.substring(0, 100) + "..." || "코스 정보가 없습니다.");
					$card.append($description);

					var $actions = $("<div>").addClass("course-actions");
					
					var $selectBtn = $("<button>").addClass("action-btn select-btn").text("코스 선택").attr("data-id", course.walking_id);
					...
					$translateBtn.click(function(e) {
						...
						$.ajax({
							...
							contentType: "application/json",
							data: JSON.stringify({
								walking_info: originalText,
								walking_name: nameText,
								walking_distance: distanceText,
								rank_text: rankText,
	...
	function bookmarkCourseFromList(walkingId, walkingName) {
		$.ajax({
			type: "POST",
			url: "addbookmarkwalking",
			contentType: "application/json",
			data: JSON.stringify({ walking_id: walkingId, walking_name: walkingName }),
			success: function(res) {
