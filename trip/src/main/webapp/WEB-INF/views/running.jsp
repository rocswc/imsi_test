...
<body>
		...
		<div class="left-section">
			<div class="walking-course">
				...
				<div class="info-form">
					...
					<br>
					<div class="info-item">
						<label for="course-name">러닝코스명</label> 
	...
	function selectCourseFromList(id) {
		...
		for (i = 0; i < allCourses.length; i++) {
			if (allCourses[i].running_id === parseInt(id)) {
				break;
			}
		}
		...
		document.getElementById("walking-id").value = course.running_id;
		document.getElementById("course-name").value = course.running_name;
		document.getElementById("course-distance").value = course.running_distance;
		document.getElementById("course-info").innerText = course.running_info;

		var newCenter = new kakao.maps.LatLng(course.running_latitude, course.running_longitude);
		map.setCenter(newCenter);

		var selectedMarker = new kakao.maps.Marker({
			position: newCenter,
			map: map,
			title: course.running_name
		});

		$.ajax({
			type: "POST",
			url: "insertRunning",
			data: JSON.stringify({ running_id: course.running_id }),
	...
	function translateCardText($card, i, res) {
		$card.find(".course-name").text(res.running_name || "");
		$card.find(".course-description").text(res.running_info || "");
		 const rawDistance = (res.running_distance || "").replace(/[^0-9.]/g, "");
		...
		$(".recommendation-header").find("h2").text("Recommended Running Courses Nearby");
	...
	navigator.geolocation.getCurrentPosition(function(position) {
		...
		$.ajax({
			type: "GET",
			url: "runningMap",
			data: { running_latitude: userLat, running_longitude: userLon },
			dataType: "json",
			success: function(response) {
				allCourses = response;
				response.forEach(function(course) {
					const walkingMarker = new kakao.maps.LatLng(course.running_latitude, course.running_longitude);
					const marker = new kakao.maps.Marker({
		...
		$.ajax({
			...
			success: function(response) {
				...
				for (var i = 0; i < response.length; i++) {
					...
					var $name = $("<h3>").addClass("course-name").text(course.running_name);
					...
					var distanceText = course.running_distance ? course.running_distance + "km" : "정보 없음";
					...
					$details.append($distance).append($rank);
					$card.append($details);

					var $description = $("<div>").addClass("course-description").text(course.running_info?.substring(0, 100) + "..." || "코스 정보가 없습니다.");
					$card.append($description);

					var $actions = $("<div>").addClass("course-actions");
					
					var $selectBtn = $("<button>").addClass("action-btn select-btn").text("코스 선택").attr("data-id", course.running_id);
					...
					$translateBtn.click(function(e) {
						...
						$.ajax({
							type: "POST",
							url: "translaterunning",
							contentType: "application/json",
							data: JSON.stringify({
								running_info: originalText,
								running_name: nameText,
								running_distance: distanceText,
								rank_text: rankText,
	...
	function selectCourse(course) {
		$("#walking-id").val(course.running_id);
		$("#course-name").val(course.running_name);
		$("#course-distance").val(course.running_distance);
		$("#course-info").text(course.running_info);
		$.ajax({
			type: "POST",
			url: "insertRunning",
			data: JSON.stringify({ running_id: course.running_id }),
	...
	function bookmarkCourseFromList(runningId, runningName) {
		$.ajax({
			type: "POST",
			url: "addbookmarkrunning",
			contentType: "application/json",
			data: JSON.stringify({ running_id: runningId, running_name: runningName }),
			success: function(res) {
	...
	$("#translate-btn").click(function() {
		const param = {
				running_id: parseInt($("#walking-id").val()),
				running_name: $("#course-name").val(),
				running_distance: $("#course-distance").val(),
				running_info: $("#course-info").text()
		};
		$.ajax({
			type: "POST",
			url: "translaterunning",
			...
			success: function(data) {
				$("#course-name").val(data.running_name);
				$("#course-distance").val(data.running_distance);
				$("#course-info").text(data.running_info);
