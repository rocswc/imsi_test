...
<div class="main-wrapper">
  ...
  <div class="main-content">
    ...
    <div class="container">
      <div class="tabs">
        <button class="tab active">나의 등산코스</button>
      </div>
      <div class="content">
		      ...
		      if (res.status === "success") {
		        ...
		        for (var i = 0; i < list.length; i++) {
		          ...
		          var map = new kakao.maps.Map(document.getElementById(mapId), {
		            center: new kakao.maps.LatLng(item.hiking_latitude, item.hiking_longitude),
		            level: 4
		          });

		          new kakao.maps.Marker({
		            map: map,
		            position: new kakao.maps.LatLng(item.hiking_latitude, item.hiking_longitude),
		            title: item.hiking_name
		          });
		        }
		      } else {
	...
	$(document).on("click", ".action-btn", function () {
	    ...
	    const hikingName = $item.find(".bookmark-title").text();
	   const hikingId = Number($item.find(".hiking-id-hidden").val()); // ✅ 정확하게 추출

	    if (!confirm(`"${hikingName}" 북마크를 삭제하시겠습니까?`)) return;

	    $.ajax({
	        type: "POST",
	        url: "deletebookmarkhiking",
	        data: JSON.stringify(hikingId),
